...
# Filter rows where the Label column contains "!!Estimate"
...
gini_estimates_all_labels <- colnames(gini_estimates_all) <- stringi::stri_replace_all_fixed(
  colnames(gini_estimates_all),
  "!!Estimate",
  "",
  vectorize_all = FALSE
)
...
print(gini_estimates_all_labels)

# Load tidyr for pivoting
library(tidyr)

# Pivot from wide to long format (states as one column, estimates as another)
gini_long <- gini_estimates_all %>%
  pivot_longer(cols = everything(), 
               names_to = "State", 
               values_to = "Gini_Estimate")

# Remove the "!!Estimate" from the State column name (cleaned earlier)
gini_long$State <- sub("!!.*", "", gini_long$State)
