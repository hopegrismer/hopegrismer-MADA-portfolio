...
---
## Fun Background on my Data - Paraphrased from Original Published Article
...
```
Above is the graph that I attempted to recreate from the published article; here: https://projects.fivethirtyeight.com/college-fight-song-lyrics/

I asked AI to create a four quadrant graph with an x-axis that represents duration and a y axis that represents the beats per minute of each school's reported data. I told the model that I had the .csv raw data. I also specified that the dash lines that divide it into four quadrants represents the corresponding for averages for duration and beats per minute. Additionally, the prompt included a specific quadrant text label.
...
```{r}
# Rename the columns in the summary_table dataframe
colnames(summary_table) <- c("School", 
                             "Duration (Seconds)", 
                             "Beats Per Minute", 
                             "Average Duration (Seconds)", 
                             "Average Beats Per Minute", 
                             "BPM Comparison (Quadrants)", 
                             "Duration (Seconds) Relative to Mean Duration", 
                             "BPM Difference from Mean")

# Create the table with kable and style it
summary_table %>%
  ...
  column_spec(1, bold = TRUE, color = "white", background = "#0073e6") %>%  # Header row with blue background
  column_spec(2, color = "darkblue") %>%
  column_spec(3, color = "darkred") %>%
  column_spec(4, color = "green") %>%  # Column for average duration
  column_spec(5, color = "orange") %>%  # Column for average bpm
  column_spec(6, color = "purple") %>%  # Column for bpm comparison
  column_spec(7, color = "darkgreen") %>%  # Column for duration difference
  column_spec(8, color = "brown") %>%  # Column for bpm difference
  # Adjusted header row to reflect 8 columns and updated headings
  add_header_above(c("Fight Song Details" = 4, "Comparison" = 4)) %>%  # Two groups: 4 columns for details, 4 for comparison
  row_spec(0, bold = TRUE, background = "#d9d9d9") %>%  # Bold header with light grey background
  row_spec(1:nrow(summary_table), color = "black", background = ifelse(summary_table$bpm_comparison == "Above Average", "#e6f7ff", "#ffe6e6")) %>%  # Soft blue for above average and soft red for below
  footnote(general = "The above table summarizes key metrics of the fight songs, including their duration, BPM, and how they compare to the average-- both categorically and quanitatively.", 
